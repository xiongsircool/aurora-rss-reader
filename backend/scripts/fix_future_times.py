    ...
    if title:
        ...
        if year_match:
            ...
            if year <= current_year:
                try:
                    # 设定为该年的6月1日（避免使用1月1日，因为有些期刊可能还没出版）
                    result = datetime(year, 6, 1, tzinfo=timezone.utc)
                    logger.warning("从标题提取年份作为时间: %s -> %s", title, result)
                    return result
                except Exception as e:
                    logger.debug("从标题构建时间失败: %s", e)

    logger.debug("无法解析时间，所有方法均失败")
    return None


async def fix_future_times():
    """修复未来时间的条目"""
    logger.info("开始修复未来时间的条目...")

    with SessionLocal() as session:
        # 获取所有未来时间的条目
        future_entries = session.exec(
            select(Entry).where(Entry.published_at > datetime.now())
        ).all()

        logger.info(f"找到 {len(future_entries)} 个未来时间的条目")

        if not future_entries:
            logger.info("没有未来时间的条目需要修复")
            return
        ...
        logger.info(f"需要重新解析 {len(feed_urls)} 个订阅源")
                        ...
                        if item_url and item_url in url_to_entry:
                            ...
                            if parsed_time:
                                ...
                                if parsed_time <= datetime.now():
                                    ...
                                    logger.info(f"修复时间: {entry.title[:50]}... -> {parsed_time}")
                                else:
                                    logger.warning(f"解析出的时间仍然是未来时间: {entry.title[:50]}... -> {parsed_time}")
                            else:
                                # 如果无法解析时间，考虑删除published_at（设置为None）
                                entry = url_to_entry[item_url]
                                old_time = entry.published_at
                                entry.published_at = None
                                session.add(entry)
                                removed_count += 1
                                logger.info(f"清除无效时间: {entry.title[:50]}... (原时间: {old_time})")
        ...
        logger.info(f"修复完成！")
        logger.info(f"- 成功修复: {fixed_count} 个条目")
        logger.info(f"- 清除无效时间: {removed_count} 个条目")

        # 统计修复后的情况
        future_entries_after = session.exec(
            select(Entry).where(Entry.published_at > datetime.now())
        ).all()
        ...
        logger.info(f"修复后未来时间条目数: {len(future_entries_after)}")
        logger.info(f"时间解析成功率: {len(with_published_time)/len(total_entries)*100:.1f}%")


if __name__ == "__main__":
    asyncio.run(fix_future_times())