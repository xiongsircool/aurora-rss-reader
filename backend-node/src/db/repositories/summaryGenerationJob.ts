...
import { SummaryGenerationJob, generateId } from '../models.js';

export interface SummaryGenerationJobCreateInput {
  entry_id: string;
  language: string;
  max_attempts?: number;
...
export class SummaryGenerationJobRepository {
  private db = getDatabase();

  createOrReset(input: SummaryGenerationJobCreateInput): SummaryGenerationJob {
    const now = new Date().toISOString();
    const existing = this.findByEntryIdAndLanguage(input.entry_id, input.language);

    if (existing) {
      const updated: SummaryGenerationJob = {
        ...existing,
        status: 'queued',
        attempts: 0,
      ...
      this.db.prepare(`
        UPDATE summary_generation_jobs SET
    ...
    this.db.prepare(`
      INSERT INTO summary_generation_jobs (
        id, entry_id, language, status, attempts, max_attempts, next_run_at,
  ...
  findByEntryIdAndLanguage(entryId: string, language: string): SummaryGenerationJob | null {
    const row = this.db.prepare(
      'SELECT * FROM summary_generation_jobs WHERE entry_id = ? AND language = ?'
    ).get(entryId, language) as SummaryGenerationJob | undefined;
    return row ?? null;
  }

  leaseDueJobs(limit: number, owner: string): SummaryGenerationJob[] {
    const now = new Date().toISOString();
    ...
    const transaction = this.db.transaction(() => {
      const candidates = this.db.prepare(`
        ...
        LIMIT ?
      `).all(now, limit) as SummaryGenerationJob[];

      const leased: SummaryGenerationJob[] = [];
