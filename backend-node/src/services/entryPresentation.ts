...
function normalizeUrl(value: string | null | undefined, baseUrl: string | null): string | null {
...
function looksLikeJunkImage(url: string, alt: string | null, width: number | null, height: number | null): boolean {
  ...
  if (junkTokens.some((token) => lowerUrl.includes(token) || lowerAlt.includes(token))) {
    return true;
  }
...
function extractImagesFromHtml(html: string | null, baseUrl: string | null): EntryMediaImage[] {
  ...
  try {
    const nodes = Array.from(dom.window.document.querySelectorAll('img'));
    for (const node of nodes) {
      ...
      if (looksLikeJunkImage(url, alt, width, height)) {
        continue;
      }
...
function addImageCandidate(images: EntryMediaImage[], candidateUrl: string | null, baseUrl: string | null): void {
  ...
  if (looksLikeJunkImage(url, null, null, null)) {
    return;
  }
