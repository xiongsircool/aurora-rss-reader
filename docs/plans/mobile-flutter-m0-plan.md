...
- [x] 端上抓取文章 HTML
- [x] Mozilla Readability 正文提取和危险标签清洗
- [x] 提取失败时回退 Feed 内容
...
- [x] 50,000 条 SQLite 基准有可复现报告(主机完成,真机仍需复测)
- [x] 数据库迁移和解析测试全绿
- [x] 已记录平台差异与不可行项
- [x] 确认直接依赖许可可用于 GPLv3 项目

## 8. M1 预告

M1 本地阅读闭环已经实现:订阅地址输入、端上抓取解析、SQLite 落库、混合收件箱、全部/未读筛选、已读、收藏、分页、单源/全量刷新、删除订阅、本地 FTS 搜索、静态 HTML 阅读页、安全外链、持久化代理设置、OPML 导入/导出以及 Mozilla Readability 网页全文提取与缓存已接入 UI。AI、自托管连接和完整视觉打磨不进入 M1。
